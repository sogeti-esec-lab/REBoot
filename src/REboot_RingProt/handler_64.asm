...
gphandler64_start:		
		...
		mov			rbx, [rsp + 10h + 28h]				; Get where the fault occured
		
		; Last case is LGDT
		; 0000000000470D04 0F 01 10    lgdt    fword ptr [rax]
		; before jumping to ntoskrnl.exe
		
		cmp		word ptr [rbx], 0CB48h
		jnz		not_retf
		
; retf case
		mov		dword ptr [esp + 018h + 28h], 08h
		mov		dword ptr [esp + 030h + 28h], 030h	
		jmp		gphandler64_restore_end		
		
not_retf:
		
		cmp		word ptr [rbx], 010Fh
		jnz		not_lgdt
	
		cmp		byte ptr [rbx + 2], 10h
		jnz		not_lgdt		
		
lgdt_case:
		; - Update CS and SS
		; - Get LOADER_PARAMETER_BLOCK from r12
		; - Inject into kdcom.dll
		
		mov		dword ptr [esp + 018h + 28h], 08h
		; UPDATE SS
		mov		dword ptr [esp + 030h + 28h], 00h
...
not_lgdt:
		...
		; Test case lidt fword [rcx]
		cmp		word ptr [rbx], 010Fh
		jnz		not_lidt
	
		cmp		byte ptr [rbx + 2], 19h
		jnz		not_lidt

lidt_case:

		mov		word ptr [rcx], 2C0h
		jmp		not_jmp_far

not_lidt:
		cmp		byte ptr [rbx], 0ECh
		jnz		not_in
		
		add		dword ptr [rsp + 10h + 28h], 1h
		jmp		gphandler64_restore_end		
		
not_in:
		cmp		byte ptr [rbx], 0EEh
		jnz		not_in_out
...
not_in_out:
		; Test case when return to 32bit then to 16 bit.
		; 66 8E D8	db 66h ; mov     ds, ax
		cmp			word ptr [rbx], 8E66h
		jnz			not_mov_ds_ax
	
		cmp			byte ptr [rbx + 2], 0D8h
		jnz			next_mov
		
mov_ds_ax_case:
		; Restore ring 0 correctly
		; UPDATE CS
		mov			dword ptr [esp + 018h + 28h], 08h
		; UPDATE SS
		mov			dword ptr [esp + 030h + 28h], 030h	
...
next_mov:
		; 66 8E D0 			mov ss, ax
		cmp			byte ptr [rbx + 2], 0D0h
		jnz			not_mov_ds_ax		

mov_ss_ax_case:
		; Do not update SS
		
		add			dword ptr [rsp + 10h + 28h], 3h
		jmp			gphandler64_restore_end			
		
not_mov_ds_ax:
		; Test case when come back to 64 bit from 32
		cmp			byte ptr [rbx], 0EAh	
		jnz			not_jmp_far
		
jmp_far_case:
		mov			rbx, [rbx + 1]
		mov			dword ptr [esp + 10h + 28h], ebx
		mov			dword ptr [esp + 18h + 28h], 11h
		mov			dword ptr [esp + 30h + 28h], 19h
		
		jmp			gphandler64_restore_end

not_jmp_far:
		; What's the length of the privilegied instruction ?
		mov 		edx, 64
		mov			rcx, rbx
		call		LDE
		
		; update IP, to go after privilegied instruction
		add			[rsp + 10h + 28h], rax
		
		push		rax
		call		prepare_exec_privi_instru
		
gphandler64_restore_end:
		pop			rsi
		pop			rdi
		pop			rbx
		pop			rdx
...
prepare_exec_privi_instru:

		pop			rdi
		add			rdi, (exec_privi_instru - gphandler64_restore_end)
		mov			rcx, 14h
		mov			rax, 90h
		rep			stosb
...
restore_save_buffer:
		...
		sub			rdi, 14h
		mov			rsi, rbx
		pop			rcx					; Get saved size of privilegied instruction
		rep			movsb

restore_register:
		; restaure context before execution of privilegied instruction
		pop			rsi
		pop			rdi
		pop			rbx
		pop			rdx
...
;
; release : 1.7 - 07-23-10 - bugfix on pushfd instruction (thanks to Stefan Bühlmann) 
